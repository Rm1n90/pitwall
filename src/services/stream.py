...
class TelemetryStreamClient(QThread):
  ...
  def _connect_to_server(self):
    ...
    try:
      ...
      self.error_occurred.emit(f"Connection timeout - is Pitwall running?")
      raise
    except ConnectionRefusedError:
      self.error_occurred.emit(f"Connection refused - is Pitwall running on {self.host}:{self.port}?")
      raise
