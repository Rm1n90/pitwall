    ...
    def _draw_info_box(self, window, code, driver_pos, center_y, box_width, box_height):
        ...
        if lb is not None and getattr(lb, "practice_mode", False):
            ...
            try:
                ...
                # Entries carry race progress in laps, so a gap becomes a time
                # using the leader's own lap time rather than an assumed speed.
                lap_seconds = getattr(lb, "reference_lap_s", 92.0) or 92.0
                lap_metres = getattr(window, "circuit_length_m", None) or 5000.0

                def get_gap_str(neighbor_idx, prefix, sign):
                    n_code, _, _, n_pos = lb.entries[neighbor_idx]
                    gap_laps = abs(curr_pos - n_pos)
                    time = gap_laps * lap_seconds
                    dist = gap_laps * lap_metres
